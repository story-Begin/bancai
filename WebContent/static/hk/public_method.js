function uploadImg(){
    ...
    uploader.on('uploadSuccess', function( file,json) {
        ...
        if("1"==json.res){
            ...
            $("#picurl").val(savePath);
            $("#xctp").attr("src",jypath+savePath);
            $("#picurl2").val(savePath);
        }else{
...
function uploadFile(){
    ...
    uploader.on( 'uploadSuccess', function(file,json) {
        ...
        if("1"==json.res){
            ...
            $("#filenames").append('<div>'+file.name+'<i class="icon-remove-sign del" style="margin-left:5px;font-size:20px;color:rgba(209, 46, 46, 1);cursor:pointer;"></i></div>');
        }else{
            JY.Model.error(json.resMsg);
            var $li = $( '#'+file.id ),$error = $li.find('div.error');
            // 避免重复创建

...
function SetProgressTime(fun, startTime, endTime) {
    ...
    var indexStart2 =Year+"-"+Month+"-" +currentDate + " " + Hours + ":" + Minutes+":"+Seconds;
    var indexStart3 =Year+"-"+Month+"-" +currentDate + " " + Hours + ":" + Minutes+":"+Seconds;
...
var ScrollBar = {
    ...
    Value: function () {
        ...
        $("#scrollBarBox").click(function (event) {
            $("#progressTime_control").attr("title","暂停") ;
            var now=$(".timeipt1").val();
            var nyr = (now.split(' ')[0]);
            start_time= $(".timecode").text();
            //var seconds=new Date($(".timeipt2").val().replace(/-/g, '/'))-new Date($(".timeipt1").val().replace(/-/g, '/'));
            //var df=seconds/1000/100;
            //var h_m_s=new Date(start_time.replace(/-/g, '/')).getHours()*60*60+new Date(start_time.replace(/-/g, '/')).getMinutes()*60+new Date(start_time.replace(/-/g, '/')).getSeconds();
            //var con=Math.ceil(h_m_s/df);
            //document.getElementById("pathOcx").SetPlayBackPos(con);
            //参数时间类型设置成整形 int
           // var dTime = new Date(start_time.replace(new RegExp("-", "g"), "/"));
           // var lTime =parseInt(dTime.getTime()/1000);//秒级
           // var ret= document.getElementById("pathOcx").SetPlaybackTime(lTime);
          var ret= document.getElementById("pathOcx").SetPlayBackAbsTime(start_time);
           if(ret==-1){ alert("当前时间没有历史视频,请往后点击播放轴！");}
           else {
            ...
            //document.getElementById("pathOcx").StartPlayBack(JSON.stringify(backPlayInfo()));
        ...
        $("#scrollBarBox").mousemove(function (event) {
            var changeX = event.clientX - ScrollBar.currentX;
            currentValue = changeX - ScrollBar.currentX - $("#scrollBar").offset().left;
            $(".timecode").show().css("left", currentValue -50 + "px");
            if ((currentValue + 1) >= $("#scrollBar").width()) {
                $(".timecode").css("left", $("#scrollBar").width() - 110 + "px");
                ScrollBar.value = ScrollBar.maxValue;
            } else if (currentValue <= 0) {
                $(".timecode").css("left", "-50px");
                ScrollBar.value = 0;
            } else {
                ScrollBar.value = Math.round(currentValue * ScrollBar.maxValue / $("#scrollBar").width());
            }
...
function SetTime(value) {
    ...
    var weekArray = new Array("周日", "周一", "周二", "周三", "周四", "周五", "周六");
    var week = weekArray[new Date(startDate).getDay()];
    var indexStart1 = Year+"-"+month+"-" +currentDate + " " + Hours + ":" + Minutes+":"+Seconds;
...
function progressTimeControl(img) {
    if ($(img).attr("title") == "暂停") {
        ...
        $(".rb-btn3").css("display","block");
        $(".rb-btn4").css("display","none");
        window.clearInterval(_mProgressTimer);
    }else {
        $(img).attr("title", "暂停");
        $(".rb-btn3").css("display","none");
        $(".rb-btn4").css("display","block");

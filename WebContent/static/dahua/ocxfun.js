...
function historyRealplay(cameraId) {
	...
	var nRet = obj.DPSDK_StartRealplayByWndNo(gWndId, nWndNo, szCameraId, "1", "1", "1");
...
function ButtonCreateWnd_onclick(n)
{
	var obj = document.getElementById("DPSDK_OCX");
	// var nWndCount = document.getElementById("textWndNum").value;
	obj.DPSDK_SetWndCount(gWndId, n);
...
function ButtonLogin_onclick()
{
	...
	var nRet = obj.DPSDK_Login("10.25.73.11", "9000", "system", "admin123456789");
...
function ButtonAsyncStartRealplayByWndNo_onclick()
{
	...
	var nWndNo = obj.DPSDK_GetSelWnd(gWndId);
	var nRealFlag = obj.DPSDK_AsyncStartRealplayByWndNo(gWndId, nWndNo, szCameraId, "1", "1", "1");
	//ShowCallRetInfo(nRet, "播放视频");
	//if(nRet == 0)
	//{
	//	insertRealplayHistory(szCameraId);
...
function ButtonStartRealplayByHWND_onclick()
{
	...
	var	nRet = obj.DPSDK_StartRealplayByHWND(hWnd, szCameraId, "1", "1", "1");
	ShowCallRetInfo(nRet, "播放视频");
	if(nRet == 0)
	{
		insertRealplayHistory(szCameraId);
...
function ButtonCapturePictureByWndNo_onclick()
{
	...
	var mydate=new Date();
	var reg=new RegExp(":","g");
	var path="c:\\dahua\\dahuaphoto\\"+mydate.toLocaleString().replace(" ","").replace("年","").replace("月","").replace("日","").replace(reg,"")+".bmp";
	// alert("存储路径："+path)
...
function ButtonDownloadRecordByTime_onclick()
{
	...
	var mydate=new Date();
	var reg=new RegExp(":","g");
	gRecordPath="c:\\dahua\\dahuavideo\\"+mydate.toLocaleString().replace(" ","").replace("年","").replace("月","").replace("日","").replace(reg,"")+".dav";
	alert("存储路径："+gRecordPath);
...
{
	var obj = document.getElementById("DPSDK_OCX");
	var strRecordPathName = "c:\\dahua\\dahuavcr\\record.dav";
	var nWndNo = obj.DPSDK_GetSelWnd(gWndId);
	// var fileSave = new ActiveXObject("MSComDlg.CommonDialog");
	// fileSave.MaxFileSize = 128;
	// fileSave.ShowSave();
	// if(fileSave.fileName == "")
	// 	return -1;
	ShowCallRetInfo(obj.DPSDK_StartRealRecordByWndNo( gWndId, nWndNo,strRecordPathName), "开始录像");
}
...
function ButtonStartRealplayByFreeWnd_onclick()
{
	...
	var nRet = obj.DPSDK_StartRealplayByFreeWnd(gWndId, szCameraId, "1", "1", "1");
	...
	function dragFun(item){
		...
		oDrag.onmousedown=function(ev){
			...
			document.onmousemove=function(ev){
				...
				num = 9*(l+8)/100 + 1;
				num = num.toFixed(2);
				selectCameraStep=Math.floor(num);
			};

...
function realmonitor() {
  ...
  if (win_len== 1) {
    ...
    oVideoControl.event.realmonitor({
      snum: winIndex % win_len,
      path: rtsp_url
    }, function (data) {
      console.log(data);
      if (winIndex == win_len-1)
        winIndex = 0;
      else
        winIndex++;
    })
    console.log(winIndex+"1111"+rtsp_url);
  }
}
...
//   },function(data) {
//      len = data.data.windowState.length;
//     return len;
//   })
//
// }
...
function getRealmonitorRtsp(cameraId) {
  var host = $('#host').val() || '10.35.81.209';
...
function captureBtn() {
  ...
  // var url="http://10.25.73.21:8314/admin/rest/device/rest/getManualCaptureEx";
  var params = {

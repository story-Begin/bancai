;(function ($, _) {
    ...
    kendo.ui.TabStrip.fn.addTab = function (item) {
        ...
        // 默认是newWindow模式
        var defaultMode = 'tabView';

        var mode = $('#toggle-view-mode').data('mode') || defaultMode;

        if(item.title=="VMYL01"||item.title=="VMYL02"||item.title=="VMYL04"||item.title=="VMYL06"||item.title=="SY"||item.title=="GSMP01")
            mode="newWindow";
        ...
        if (mode === 'tabView') {
            var parms = {"formEname":item.title};
            var title = "";
            $.ajax({
                url:ctx+"/backstage/cx/deviceAlarm/getTablePageName",
                async: false,
                type:"POST",
            ...
            var iframe = $('<iframe>');
            console.log($(last));
            console.log($(lastContentElement));
            ...
            iframe.attr('overflow-y', 'hidden');
            iframe.attr('frameborder', '0');
            // iframe.attr('height', $(window).height() - 120); // iframe的高度
            iframe.attr('height', $(window).height() - 90);
